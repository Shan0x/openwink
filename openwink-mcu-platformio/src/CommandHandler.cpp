...
void CommandHandler::parseCustomCommand(string command) {
  ...
  for (string part : commandSequence) {
    ...
    // espidf TODO: Replace Serial with ESP_LOGI
  }
}
...
void CommandHandler::handleQueuedCommand() {
  ...
  // Both Blink
  case 3:
    // Should function regardless of current headlight position (ie: Left is up,
    // right is down -> Blink Command -> Left Down Left Up AND Right Up Right
    // Down)
    bothBlink();
    break;
...
void CommandHandler::handleQueuedCustomCommand() {
  ...
  for (int i = 0; i < commandSequence.size(); i++) {
    ...
    if (cmd[0] == 'd') {
      ...
      // Both Blink
      case 3:
        // Should function regardless of current headlight position (ie: Left is
        // up, right is down -> Blink Command -> Left Down Left Up AND Right Up
        // Right Down)
        bothBlink();
        break;
    ...
    if (i == (commandSequence.size() - 1) &&
        CommandHandler::custom_command_loop)
      i = -1;
  }
