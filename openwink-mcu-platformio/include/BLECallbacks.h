...
enum AuthState { UNCLAIMED, WAIT_CLAIM, WAIT_TOKEN, AUTHENTICATED };
...
class ServerCallbacks : public NimBLEServerCallbacks {
  ...
  void onPhyUpdate(NimBLEConnInfo &connInfo, uint8_t txPhy,
                   uint8_t rxPhy) override;
  void onMTUChange(uint16_t MTU, NimBLEConnInfo &connInfo) override;
};

class LongTermSleepCharacteristicCallbacks
...
class CustomButtonPressCharacteristicCallbacks
    : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic *pChar, NimBLEConnInfo &info) override;
  void onRead(NimBLECharacteristic *pChar, NimBLEConnInfo &info) override;
};

class HeadlightBypassCharacteristicCallbacks
    : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic *pChar, NimBLEConnInfo &info) override;
};

class HeadlightOrientationCharacteristicCallbacks
    : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic *pChar, NimBLEConnInfo &info) override;
};
...
class CustomCommandCharacteristicCallbacks
    : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic *pChar, NimBLEConnInfo &info) override;
};
